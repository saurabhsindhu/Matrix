...
#define ZOOM_FACTOR  0.0f
#define ALPHA_FACTOR 0.01f
...
//initial view coordinate (slide if needed)

- (void)prepareLayout
{
    [super prepareLayout];
	
	self.centerOffset = (self.collectionView.bounds.size.width) * (0.055f);
...
- (CGSize)collectionViewContentSize
{
    const CGSize theSize = {
        .width = self.cellSpacing * self.cellCount + self.centerOffset * .01f,
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect
{
	...
	// Cells...
	// TODO --  make sure we get all cells... Ideally we should compute the right number of extra cells to fetch...
    NSInteger start = MIN(MAX((NSInteger)floorf(CGRectGetMinX(rect) / self.cellSpacing), 0), self.cellCount);
